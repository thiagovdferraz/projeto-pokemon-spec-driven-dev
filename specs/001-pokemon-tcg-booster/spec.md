...
- **FR-005b**: Upon completing the download, `download_cards.py` MUST generate (or overwrite)
  `assets/index.json` — a machine-readable manifest listing every downloaded card with its
  `id`, `name`, `rarity` label, and `folder` path. The game MUST load this file at startup
  to build the rarity pools, since the browser cannot enumerate filesystem directories.
...
- **FR-010**: Cards MUST be revealed sequentially (slot 1 → slot 6), each with a CSS 3D flip
  animation (face-down → 180° rotateY → face-up) implemented without external animation
  libraries. The next card's animation MUST NOT begin until the current flip completes.
...
- **FR-013**: All 6 cards of a booster MUST be automatically added to the player's persistent
  collection in `localStorage` at the moment the last reveal animation completes — no
  confirmation step is required. The "Open Another Booster" control MUST become available
  immediately after this save.
- **FR-014**: The collection view MUST display each distinct card obtained, showing the
  card image, card name, rarity, and number of copies owned. Cards MUST be sorted by
  their set number (Pokédex / canonical set order from `assets/index.json`) by default.
  The active rarity filter does not change the sort order.
- **FR-015**: The collection view MUST support filtering cards by rarity (one rarity at a
  time, plus an "All" option).
- **FR-015b**: The collection view MUST display a progress panel showing: total cards collected
  vs. total cards in the set (e.g., "87/165"), and per-rarity-tier completion counts
  (e.g., "Lendárias: 0/6"). These counters MUST update immediately after each pack opening.
...
- **Rarity Pool**: The set of all available cards in a given rarity tier, loaded at runtime
  from `assets/index.json` (generated by `download_cards.py`). The browser MUST NOT attempt
  to enumerate filesystem directories directly.
...
- **SC-001**: Running `download_cards.py` completes without errors and produces all 7 rarity
  folders populated with card images for every card in the 151 set, plus a valid
  `assets/index.json` containing one entry per downloaded card.
...
- **SC-008**: After opening packs, the progress panel reflects exact per-rarity counts
  immediately (zero discrepancy between displayed counters and actual localStorage state).
...
- Q: Como o browser descobre quais cartas existem em cada pasta de raridade em tempo de execução? → A: `download_cards.py` gera `assets/index.json` com id, nome e raridade de cada carta baixada; o jogo carrega esse manifesto no startup para montar os pools de raridade.
- Q: O que acontece com as cartas após a revelação completa do booster? → A: Todas as 6 cartas são adicionadas automaticamente à coleção ao fim da animação, sem botão de confirmação; o jogador pode abrir o próximo pacote imediatamente.
- Q: Qual o estilo da animação de revelação de cada carta? → A: Flip 3D em CSS — carta começa virada para baixo e gira 180° revelando a face, implementado com CSS transform/rotateY sem bibliotecas externas.
- Q: Qual a ordenação padrão da coleção? → A: Número da carta no set (ordem do Pokédex / ordem canônica do set sv3pt5), facilitando identificar cartas faltantes.
- Q: Quais estatísticas de progresso são exibidas ao jogador? → A: Progresso de conclusão por tier de raridade + contador global (ex: "87/165 cartas — Lendárias: 0/6"), visível na tela de coleção.
